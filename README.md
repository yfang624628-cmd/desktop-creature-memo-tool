...
| | |
|---|---|
| **测评生成** | 从 20 张情境卡中选出最贴切的 5 张与无感的 3 张，映射为五个性格维度，最终匹配 20 个原型之一（各原型概率均等，各 5%）。结果页示例文案：「你一次就能做对。代价是别人总以为你还没开始。」 |
| **便签** | 内置便签板，通过两个按钮新建黄色或粉色便签，支持就地编辑、拖拽排序与删除，不设置提醒时间。两种颜色在功能上完全一致，区别仅在于宠物的视觉反应：黄色便签会堆积为地面上的纸堆（带有重量感），粉色便签则以气泡形式漂浮在头顶（不带重量感） |
| **负载** | 未完成的黄色便签会持续堆高纸堆，进而影响宠物的姿态与对话内容。系统不设隐藏数值，纸堆的高度即是全部状态呈现 |
| **提醒** | 黄色便签搁置超过 3 天会被主动提及，并标注天数；粉色便签则在第 4 天被重新提起 |
| **日程** | 日程由对应职位的任务池随机生成，另设周末爱好与周日晚间专属文案。用户的便签内容不会进入其日程安排 |
| **画面** | 16×16 像素网格动画，包含写字、改稿、打字、开会、摸鱼、吃饭、睡觉七种场景动画 |
| **图鉴与抽卡** | 共 20 个可收集形态。记录便签可获得积分（每日上限 20 分），积满 30 分可抽取一只，允许重复。切换形态不影响已有便签 |
| **即兴文案**（可选） | 状态栏后半句的补充文案，每天出现六到七次。由大模型每日批量生成，依据宠物的物种、性格、职位及当前便签堆积情况生成内容；未配置密钥时使用内置静态文案库，不影响功能完整性 |
| **撤销** | 便签删除后保留 6 秒的撤销提示，超时后自动清除 |
| **回看** | 按完成日期排列，记录的是便签被勾选完成那一刻的文本内容。由于便签支持随时编辑，回看记录保存的是完成时的快照而非实时引用，以避免后续修改影响历史记录。可以月份为单位回看每日完成的便签数量。 |
