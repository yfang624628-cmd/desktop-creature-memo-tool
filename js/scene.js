...
(function (global) {
  ...
  /* 通勤的车。三扇分开的窗 + 底下两个轮子，这两处是它跟电视、电脑的唯一区别——
   * 原来画成「圆角方块加一条亮带」，跟那两样几乎一模一样，根本读不出是车。
   * 速度线也删了：道具区从 27 列到 37 列只有 11 格，车占满之后左边只剩两格，
   * 两格画不出「一道线」，只剩两个孤点，读成噪点。动效改成窗外的景往后掠。 */
  var BUS = [
    '.########.',
    '#++#++#++#',
    '##########',
  ...
  // 通勤：车不动，窗外的景一格一格往后掠。跟电视屏幕上那个移动的暗块是同一套手法
  function drawCommute(f) {
    var top = FLOOR - BUS.length + 1;
    blitFloor(BUS, PROP_COL);
    var win = [8, 7, 5, 4, 2, 1];              // 只落在窗上，跳过中间的立柱
    put(PROP_COL + win[f % win.length], top + 1, false);
  }
